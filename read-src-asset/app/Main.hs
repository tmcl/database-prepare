...
worker = do
  ...
  let arguments =
        Data.List.foldr
          ( \arg accum -> case arg of
              theArg
                | Just outPath <- Data.List.stripPrefix "--out=" theArg -> accum {out = Just (Filesystem.Path.CurrentOS.decodeString outPath)}
                | Just schemaPath <- Data.List.stripPrefix "--schema=" theArg ->
                    accum
          ...
          (Arguments Nothing Data.Set.empty Data.Set.empty)
          args
  ...
  schemaPaths <-
    Data.Set.fromList . join <$> forM (Data.Set.toList arguments.argSchemaPaths) \dir -> do
